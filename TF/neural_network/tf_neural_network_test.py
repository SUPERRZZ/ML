...
# 定义placeholder作为存放输入数据的地方,两个输入节点
...
cross_entropy = -tf.reduce_mean(
    # tf.log()这个函数完成了对张量中的所有元素依次求对数功能
...
rdm = RandomState(1)
dataset_size = 128
# 随机生成输入样本   [4.17022005e-01 7.20324493e-01]
X = rdm.rand(dataset_size, 2)
# 定义规则来给出样本的标签。在这里所有x1+x2<1的样例都被认为是正样本（比如零件合格），而其他为负样本（比如零件不合格）   [0]
